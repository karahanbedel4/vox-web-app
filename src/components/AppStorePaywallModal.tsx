...
interface AppStorePaywallModalProps {
  isOpen: boolean;
  onClose: () => void;
  reason?: 'limit_reached' | 'pages_exceeded' | 'not_logged_in';
}
...
export const AppStorePaywallModal: React.FC<AppStorePaywallModalProps> = ({
  ...
  const handleDownloadClick = () => {
    alert('VOX iOS uygulaması indirme sayfasına yönlendiriliyorsunuz... (Sınırsız PDF ve Sesli Dinleme)');
  };
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-md bg-surface-container border-2 border-emerald-500/50 rounded-3xl overflow-hidden shadow-[0_0_60px_rgba(16,185,129,0.25)] text-on-surface"
        >
          {/* Header Image & Badge */}
          <div className="relative p-6 bg-gradient-to-b from-emerald-900/40 via-surface-container to-surface-container text-center space-y-3 border-b border-card-border">
            <button
              onClick={onClose}
              className="absolute top-4 right-4 p-2 rounded-full bg-surface-variant hover:bg-card-border text-on-surface-variant hover:text-on-surface transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 text-[11px] font-bold uppercase tracking-widest shadow-[0_0_15px_rgba(16,185,129,0.3)]">
              ...
              <span>VOX PRO & MOBİL SÜRÜM</span>
            </div>

            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-emerald-500 to-teal-400 p-0.5 mx-auto shadow-xl">
              <div className="w-full h-full bg-surface-container rounded-[14px] flex items-center justify-center text-emerald-400">
                <Smartphone className="w-8 h-8" />
              </div>
            </div>

            <h2 className="font-display text-xl font-bold tracking-tight text-white leading-snug">
              {reason === 'not_logged_in'
                ? 'PDF ve Özetleme İçin Giriş Yapın'
                : reason === 'pages_exceeded'
                ? 'Maksimum 10 Sayfa Sınırı Aşıldı'
                : 'Günlük Ücretsiz Özetleme Limitiniz Doldu'}
            </h2>
          </div>
          ...
          <div className="p-6 space-y-5 text-center">
            <div className="bg-emerald-950/40 border border-emerald-500/30 p-4 rounded-2xl space-y-2 text-left">
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs text-emerald-100 font-medium leading-relaxed">
                  "Günlük ücretsiz özetleme limitinize ulaştınız. Sınırsız PDF özetleme ve haberleri yolda sesli dinleme deneyimi için VOX iOS uygulamamızı indirin."
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 text-left text-xs">
              ...
              <div className="flex items-center gap-2 text-on-surface-variant">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                <span>Çevrimdışı kayıt ve HD Stüdyo seslendirme</span>
              </div>
            </div>
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full py-3.5 px-5 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-400 text-black font-extrabold text-sm rounded-2xl shadow-[0_0_25px_rgba(16,185,129,0.4)] hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-3"
              >
                {/* Apple Logo SVG */}
                ...
                <div className="text-left leading-tight">
                  <span className="text-[10px] opacity-80 uppercase block tracking-wider font-semibold">App Store'dan İndir</span>
